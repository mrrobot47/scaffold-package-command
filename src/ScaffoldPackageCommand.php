...
/**
 * Scaffolds EE commands with basic skeleton, full README.md, and more.
 * 
 * ## EXAMPLES
 *
 *     # Scaffolds EE commands with basic skeleton, full README.md, and more.
 *     $ ee scaffold package author/command
 *
 * @package ee-cli
 */
class ScaffoldPackageCommand {
	...
	/**
	 * Generate the files needed for a basic EE command.
	 *
	 ...
	 * EE `packages/local/` directory.
	 *
	 * ## OPTIONS
	 *
	 * <name>
	 * : Name for the new package. Expects <author>/<package> (e.g. 'EE/scaffold-package').
	 *
	 ...
	 *
	 * [--dir=<dir>]
	 * : Specify a destination directory for the command. Defaults to EE's `packages/local/` directory.
	 *
	 * [--license=<license>]
	 ...
	 *
	 * [--require_EE=<version>]
	 * : Required EE version for the package.
	 * ---
	 * default: ^1.1.0
	 * ---
	 *
	...
	public function package( $args, $assoc_args ) {
		...
		if ( 2 !== count( $bits ) || empty( $bits[0] ) || empty( $bits[1] ) ) {
			EE::error( "'{$assoc_args['name']}' is an invalid package name. Package scaffold expects '<author>/<package>'." );
		}

		if ( ! empty( $assoc_args['dir'] ) ) {
			$package_dir = $assoc_args['dir'];
		} else {
			$package_dir = EE::get_runner()->get_packages_dir_path() . 'local/' . $assoc_args['name'];
		}
...
EOT;
		...
		if ( empty( $files_written ) ) {
			EE::log( 'All package files were skipped.' );
		} else {
			EE::success( "Created package files in {$package_dir}" );
		}

		$force_flag = $force ? '--force' : '';
		/*if ( ! Utils\get_flag_value( $assoc_args, 'skip-tests' ) ) {
			EE::runcommand( "scaffold package-tests {$package_dir} {$force_flag}", array( 'launch' => false ) );
		}*/

		if ( ! Utils\get_flag_value( $assoc_args, 'skip-readme' ) ) {
			EE::runcommand( "scaffold package-readme {$package_dir} {$force_flag}", array( 'launch' => false ) );
		}

		/*if ( ! Utils\get_flag_value( $assoc_args, 'skip-github' ) ) {
			EE::runcommand( "scaffold package-github {$package_dir} {$force_flag}", array( 'launch' => false ) );
		}*/

		/*if ( ! Utils\get_flag_value( $assoc_args, 'skip-install' ) ) {
			EE::runcommand( "package install {$package_dir}", array( 'launch' => false ) );
		}*/
	}
	 ...
	 * ```
	 * {
	 *   "name": "EE/scaffold-package-command",
	 *    // [...]
	 *    "extra": {
	 *        "commands": [
	 *            "scaffold package",
	...
	public function package_readme( $args, $assoc_args ) {
		...
		$readme_args = array(
			...
			'required_EE_version' => ! empty( $composer_obj['require']['easyengine/easyengine'] ) ? str_replace( array( '~', '^', '>=' ), 'v', $composer_obj['require']['easyengine/easyengineEE'] ) : 'v0.0.1',
			'shields'             => '',
			'has_commands'        => false,
		...
		$readme_args['EE_requires_instructions'] = "requires EE {$readme_args['required_EE_version']} or greater";
		if ( '*' === $readme_args['required_EE_version'] ) {
			$readme_args['EE_requires_instructions'] = "requires EE's latest stable release";
		}

		if ( false !== stripos( $readme_args['required_EE_version'], 'alpha' ) ) {
			$readme_args['EE_update_to_instructions'] = 'the latest nightly release with `wp cli update --nightly`';
		}

		if ( ! empty( $composer_obj['extra']['commands'] ) ) {
			$readme_args['commands'] = array();
			$cmd_dump = EE::runcommand( 'cli cmd-dump', array( 'launch' => false, 'return' => true, 'parse' => 'json' ) );
	...
	public function package_github( $args, $assoc_args ) {
		...
		if ( empty( $files_written ) ) {
			EE::log( 'Package GitHub configuration generation skipped.' );
		} else {
			EE::success( 'Created package GitHub configuration.' );
		}
	}
	 ...
	 *
	 * EE makes use of a Behat-based testing framework, which you should use
	 ...
	 *
	 * ```
	 * Feature: Test that EE loads.
	 *
	 *   Scenario: EE loads for your tests
	 *     Given a EE install
	 *
	 *     When I run `ee eval 'echo "Hello world.";'`
	 ...
	 * * `features/load-EE.feature` is a basic test to confirm EE can
	 * load.
	 ...
	 * EE_BIN_DIR environment variable.
	 *
	 * EE Behat framework uses Behat ~2.5, which is installed with Composer.
	 *
	 * ## OPTIONS
	 *
	 * <dir>
	 ...
	 * ## EXAMPLE
	 *
	 *     ee scaffold package-tests /path/to/command/dir/
	 *
	...
	public function package_tests( $args, $assoc_args ) {
		...
		$copy_source = array(
			$EE_root => array(
				...
				'php/EE/Process.php'                      => $bootstrap_dir,
				'php/EE/ProcessRun.php'                   => $bootstrap_dir,
		...
		if ( ! glob( $features_dir . '/*.feature' ) ) {
			$copy_source[ $package_root ]['templates/load-EE.feature'] = $features_dir;
		}
		...
		foreach( $copy_source as $source => $to_copy ) {
			foreach ( $to_copy as $file => $dir ) {
				if ( 'php/EE/ProcessRun.php' === $file && ! file_exists( $source . "/{$file}" ) ) {
					continue;
				}
		...
		if ( empty( $files_written ) ) {
			EE::log( 'All package test files were skipped.' );
		} else {
			EE::success( 'Created package test files.' );
		}
	}
